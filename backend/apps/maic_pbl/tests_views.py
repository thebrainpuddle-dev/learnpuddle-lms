...
def test_happy_path_creates_session_and_returns_ws_url():
    ...
    with patch("apps.maic_pbl.views.generate_pbl_project", _fake_generate), patch(
        "apps.maic.orchestration.ai_adapter.resolve_chat_model", return_value=object()
    ):
        res = _client_for(u).post(
            "/api/maic/v2/pbl/projects/",
            data={
...
@pytest.mark.django_db
def test_create_view_passes_teacher_context_to_design_graph():
    u, _ = _make_user_with_tenant("t-context")
    captured = {}

    async def _fake_generate(cfg, _model, *, on_progress=None):
        captured["teacher_context"] = cfg.teacher_context
        return GeneratePBLResult(
            project_config={
                "projectInfo": {"title": "Contextual PBL", "description": "d"},
                "agents": [],
    ...
    with patch("apps.maic_pbl.views.generate_pbl_project", _fake_generate), patch(
        ...
        res = _client_for(u).post(
            ...
            data={
                "topic": "Water quality",
                "teacherContext": "Use class guide roles and misconception checks.",
            },
            format="json",
        )

    assert res.status_code == 201, res.json()
    assert captured["teacher_context"] == "Use class guide roles and misconception checks."
...
def test_design_loop_crash_500s_with_session_id():
    ...
    with patch("apps.maic_pbl.views.generate_pbl_project", _fake_crash), patch(
        "apps.maic.orchestration.ai_adapter.resolve_chat_model", return_value=object()
    ):
        res = _client_for(u).post(
            "/api/maic/v2/pbl/projects/",
...
def test_retrieve_no_tenant_returns_403():
    ...
    u = User.objects.create(email="orphan@dev.local", is_active=True, first_name="X")
    res = _client_for(u).get("/api/maic/v2/pbl/projects/00000000-0000-0000-0000-000000000000/")
    assert res.status_code == 403
...
def test_tenant_has_v2_access_helper_logic():
    ...
    assert tenant_has_v2_access(SimpleNamespace(is_active=False, feature_maic_v2=True)) is False
    assert tenant_has_v2_access(SimpleNamespace(is_active=True, feature_maic_v2=False)) is False
    assert tenant_has_v2_access(SimpleNamespace(is_active=True, feature_maic_v2=True)) is True
...
def test_require_tenant_v2_raises_when_disabled():
    ...
    with pytest.raises(MaicTenantError):
        require_tenant_v2(SimpleNamespace(is_active=True, feature_maic_v2=False))
    # Should not raise when flag is on
    require_tenant_v2(SimpleNamespace(is_active=True, feature_maic_v2=True))
