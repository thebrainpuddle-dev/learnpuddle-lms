...
[
  ...
  { "type": "text", "agentId": "agent-1", "content": "First, let's look at the key concept..." },
  {
    "type": "action",
    "name": "spotlight",
...
3. `type:"text"` objects contain `agentId` and `content` (speech text)
...
- `agentId` must exactly match one ID shown under "Valid discussion agentId values" in the agent roster. Do not invent agent IDs, use names, or use ids from examples.
...
Every speech object MUST include `agentId` from the agent roster exactly.
Use real handoffs: teacher lead opens the idea, student/assistant voices ask
or clarify the likely confusion, then the teacher or TA synthesizes. Do not
let one speaker read the whole slide unless only one agent is available.
