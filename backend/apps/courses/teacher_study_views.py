"""
Teacher-facing API views for AI Study Summaries.

Endpoints:
    POST   /api/v1/teacher/study-summaries/generate/           — Generate (SSE stream or cached)
    GET    /api/v1/teacher/study-summaries/                     — List teacher's summaries
    GET    /api/v1/teacher/study-summaries/<summary_id>/        — Get full summary detail
    DELETE /api/v1/teacher/study-summaries/<summary_id>/delete/ — Delete a summary
    PATCH  /api/v1/teacher/study-summaries/<summary_id>/share/  — Toggle is_shared
"""
...
from utils.decorators import teacher_or_admin, tenant_required, check_feature

logger = logging.getLogger(__name__)


class TeacherStudySummaryGenerateThrottle(UserRateThrottle):
    rate = '20/hour'


def _check_teacher_generation_throttle(request):
    """Manually check the throttle and return a 429 Response or None if OK."""
    throttle = TeacherStudySummaryGenerateThrottle()
...
def teacher_study_summary_generate(request):
    """
    Generate an AI study summary for a content item (teacher-owned).
    ...
    if not is_teacher_assigned_to_course(request.user, course):
        return Response(
    ...
    # Check for cached summary (teacher-owned)
    try:
    ...
    throttle_response = _check_teacher_generation_throttle(request)
...
@tenant_required
@check_feature("feature_maic")
def teacher_study_summary_list(request):
    """
    List the current teacher's study summaries.
...
@tenant_required
@check_feature("feature_maic")
def teacher_study_summary_detail(request, summary_id):
    """
    Get the full study summary data for a specific summary.

    Only the teacher who generated it (or an admin) can access the summary.
    """
    try:
        summary = StudySummary.objects.select_related(
            'content', 'content__module', 'content__module__course',
        ).get(pk=summary_id, generated_by=request.user)
    except StudySummary.DoesNotExist:
        return Response(
...
@tenant_required
@check_feature("feature_maic")
def teacher_study_summary_delete(request, summary_id):
    """
    Delete a study summary. Only the teacher who generated it can delete it.
    """
    try:
        summary = StudySummary.objects.get(pk=summary_id, generated_by=request.user)
    except StudySummary.DoesNotExist:
        return Response(
            {"error": "Summary not found"},
            status=status.HTTP_404_NOT_FOUND,
...
@tenant_required
@check_feature("feature_maic")
def teacher_study_summary_toggle_share(request, summary_id):
    """
    Toggle the is_shared flag on a teacher-owned study summary.

    When is_shared=True, students in the course can see this summary.
    """
    try:
        summary = StudySummary.objects.get(pk=summary_id, generated_by=request.user)
    except StudySummary.DoesNotExist:
    ...
    if summary.status != 'READY':
        return Response(
            {"error": "Only completed summaries can be shared"},
            status=status.HTTP_400_BAD_REQUEST,
        )

    summary.is_shared = not summary.is_shared
    summary.save(update_fields=['is_shared', 'updated_at'])
