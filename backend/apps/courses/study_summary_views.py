...
def _student_course_access_q(request):
    """Course access filter for shared teacher summaries visible to a student."""
    if request.user.role in ("SCHOOL_ADMIN", "SUPER_ADMIN"):
        return Q()
    return Q(content__module__course__assigned_to_all_students=True) | Q(
        content__module__course__assigned_students=request.user
    )


def _student_summary_access_q(request):
    """Summaries the current student may read."""
    return Q(student=request.user) | (
        Q(is_shared=True, status="READY", generated_by__isnull=False)
        & _student_course_access_q(request)
    )


def _shared_by(summary):
    if summary.generated_by:
        return summary.generated_by.get_full_name()
    return None


def _serialize_summary_metadata(summary):
    ...
    return {
        ...
        "shared_by": _shared_by(summary),
        "created_at": summary.created_at.isoformat(),
        "updated_at": summary.updated_at.isoformat(),
    }


def _serialize_summary_detail(summary):
    data = _serialize_summary_metadata(summary)
    data["summary_data"] = summary.summary_data
    return data
...
def student_study_summary_list(request):
    ...
    Optional query params:
      ?course_id=<uuid> to filter by course.
      ?content_id=<uuid> to filter by content.

    Includes both summaries generated by the student and READY teacher-owned
    summaries shared with students in assigned courses.
    Returns summary metadata ordered by most recent.
    """
    qs = StudySummary.all_objects.filter(
        content__module__course__tenant=request.tenant,
    ).filter(
        _student_summary_access_q(request),
    ).select_related(
    ...
    return Response([_serialize_summary_metadata(summary) for summary in qs])
...
def student_study_summary_detail(request, summary_id):
    ...
    The owning student can access their summaries. Students can also access
    READY teacher-owned summaries that were shared with students in assigned
    courses.
    """
    summary = (
        ...
        .filter(_student_summary_access_q(request))
        .select_related(
    ...
    return Response(_serialize_summary_detail(summary))
