...
function buildDefaultClassGuide(args: {
  ...
  return [
    `Learning goal: Build a ${args.sceneCount}-scene AI classroom on "${topic}" for ${audience}.`,
    'Class flow: Start with a concrete hook, teach the core ideas through worked examples, add a checkpoint after every 2-3 teaching scenes, and end with a transfer task.',
    'Teacher moves: Surface likely misconceptions early, let one student-like agent ask clarifying questions, and use the whiteboard only where step-by-step reasoning helps.',
    'PBL/activity target: When the topic benefits from doing, include one role-based task, issue-board moment, or hands-on decision instead of another static lecture.',
