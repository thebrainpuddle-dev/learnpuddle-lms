...
//
// Phase 3B enhancements: voice wave indicators, AI reasoning display,
// speed control, audio recorder for voice input, entry/exit animations.
//
// Phase 5B enhancements: Director Graph orchestration integration for
// multi-agent turn-taking during discussions.

import React, { useState, useRef, useEffect, useCallback, useMemo } from 'react';
import { motion, AnimatePresence } from 'motion/react';
import { X, Send, Loader2, MessageCircle, RotateCcw, Mic, MicOff, Gauge, Users } from 'lucide-react';
import { useMAICStageStore } from '../../stores/maicStageStore';
import { useAuthStore } from '../../stores/authStore';
import { streamMAIC } from '../../lib/maicSSE';
import { useOrchestration } from '../../hooks/useOrchestration';
import { toAgentConfig } from '../../lib/orchestration/agent-registry';
...
export const RoundtablePanel = React.memo<RoundtablePanelProps>(
  function RoundtablePanel({ sessionType, topic, agentIds, onClose }) {
    ...
    // ─── Phase 5B: Director Graph Orchestration ───────────────────────
    const {
      isRunning: isOrchestrating,
      currentAgentId: orchestratingAgentId,
      startOrchestration,
      stopOrchestration,
    } = useOrchestration({
      ...
      onTextDelta: (text, agentId) => {
        // Accumulate streaming text into a message for the current agent
        setMessages((prev) => {
          const lastMsg = prev[prev.length - 1];
          if (lastMsg && lastMsg.agentId === agentId && lastMsg.role === 'agent') {
          ...
          const agent = discussionAgents.find((a) => a.id === agentId);
          return [
            ...prev,
            {
    ...
    /** Start orchestrated multi-agent discussion on a topic */
    const handleStartOrchestration = useCallback(
      (discussionTopic: string) => {
        ...
        // Convert MAICAgent[] to AgentConfig[] for the orchestration system
        const agentConfigs = discussionAgents.map(toAgentConfig);

        const currentScene = scenes[currentSceneIndex];

        startOrchestration(agentConfigs, {
          discussionTopic,
          maxTurns: discussionAgents.length,
          slideContext: currentScene
            ? {
      ...
      [discussionAgents, scenes, currentSceneIndex, startOrchestration],
    );
      ...
      await streamMAIC({
        ...
        onEvent: (event: MAICSSEEvent) => {
          if (event.type === 'chat_message') {
            ...
            const data = event.data as { agentId: string; text?: string };
            setThinkingAgentId(data.agentId);
            if (data.text) {
      ...
      >
        <AnimatePresence>
          <motion.div
            ...
            className="flex flex-col max-w-3xl w-full mx-auto my-4 bg-white rounded-xl shadow-2xl overflow-hidden flex-1 min-h-0"
          >
            {/* Header */}
            <div className="shrink-0 flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gray-50">
              ...
              <div className="flex items-center gap-1">
                ...
                >
                  ...
                  <span className="tabular-nums font-medium">{speed}x</span>
                </button>
                <button
                  type="button"
              ...
              {discussionAgents.map((agent) => {
                ...
                return (
                  <div key={agent.id} className="flex flex-col items-center gap-1 relative">
                    ...
                    {/* Voice wave indicator for speaking agent */}
                    {isSpeaking && (
                      <VoiceWaveIndicator active color={agent.color} size="sm" barCount={5} />
                    )}
                    {/* Voice wave indicator for responding (replaces amber pulse dot) */}
                    {isResponding && !isSpeaking && (
                      <div className="absolute -top-0.5 -right-0.5">
                        <VoiceWaveIndicator active color="#f59e0b" size="sm" barCount={3} />
                      </div>
                    )}
                    <span
                      className={cn(
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3" aria-live="polite">
              {messages.length === 0 && (
                <div className="mt-4 space-y-4">
                  ...
                  {/* Orchestrated discussion button (multi-agent only) */}
                  {discussionAgents.length > 1 && sessionType === 'roundtable' && (
                    <div className="flex justify-center">
                      <button
                        type="button"
                        onClick={() => handleStartOrchestration(topic)}
              ...
              {/* Thinking indicator */}
              {(isSending || isOrchestrating) && (
                <div className="space-y-2">
                  <div className="flex gap-2 items-center">
                    {(speakingAgentId || orchestratingAgentId) ? (
                      <div className="shrink-0">
                        {(() => {
                          const activeId = speakingAgentId || orchestratingAgentId;
                          const agent = activeId ? getAgent(activeId) : null;
                  ...
                  {/* Reasoning component for thinking agent */}
                  {thinkingAgentId && (
                    <div className="ml-10">
                      <Reasoning
                        thinking={!!thinkingAgentId}
                        text={thinkingText || undefined}
                        agentName={thinkingAgent?.name}
                        agentColor={thinkingAgent?.color}
                      />
                    </div>
                  )}
                </div>
            ...
            <div className="shrink-0 px-5 py-3 border-t border-gray-100 bg-gray-50">
              ...
              {isRecording && (
                <div className="flex items-center gap-2 mb-2 px-1">
                  ...
                  <VoiceWaveIndicator active color="#ef4444" size="sm" barCount={4} />
                  <button
                    type="button"
              ...
              {isTranscribing && (
                <div className="flex items-center gap-2 mb-2 px-1">
                  ...
                  <span className="text-xs text-gray-500">Processing speech...</span>
                </div>
              )}
