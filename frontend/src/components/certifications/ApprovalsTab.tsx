// src/components/certifications/ApprovalsTab.tsx
//
// Extracted from SkipRequestsPage.tsx — self-contained tab component
// for reviewing teacher skip requests within the Certifications page.
// Provides: status tabs (All/Pending/Approved/Rejected), search,
// course-wise dropdown filter, approve/reject actions with modal.
...
interface ApprovalsTabProps {
  /** Optional: filter by a specific course ID */
  courseId?: string;
  /** Optional: list of courses for the dropdown filter */
  courses?: { id: string; title: string }[];
}

export const ApprovalsTab: React.FC<ApprovalsTabProps> = ({ courseId: initialCourseId, courses }) => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['skip-requests', activeTab, search, page, selectedCourseId],
    queryFn: () =>
  ...
  const handleReview = async () => {
    ...
    try {
      ...
      toast.error('Review failed', err?.response?.data?.error || 'Failed to submit review.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
        ...
        {courses && courses.length > 0 && (
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="rounded-lg border border-gray-300 py-2 px-3 text-sm focus:border-primary-500 focus:ring-2 focus:ring-primary-500"
            >
      ...
      {reviewItem && reviewAction && (
        ...
        >
          <div
            className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-2xl bg-white p-5 pb-6 shadow-xl sm:rounded-xl sm:p-6"
          ...
          >
            ...
            <div className="space-y-3 mb-4 bg-gray-50 rounded-lg p-4 text-sm">
              <div>
                <span className="font-medium text-gray-700">Teacher:</span> {reviewItem.teacher_name}
              </div>
              <div>
                <span className="font-medium text-gray-700">Course:</span> {reviewItem.course_title}
              </div>
              {reviewItem.certificate_url && (
                <div>
                  <span className="font-medium text-gray-700">Certificate:</span>{' '}
                  <a
                    href={reviewItem.certificate_url}
                    target="_blank"
                    rel="noopener noreferrer"
