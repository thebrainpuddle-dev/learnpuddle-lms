// src/pages/student/DiscussionPage.tsx
//
// Student discussions page — view and create threads in their section.
...
export const StudentDiscussionPage: React.FC = () => {
  ...
  const { data, isLoading, isFetching } = useQuery<ThreadsResponse>({
    queryKey: ['student-discussions', statusFilter, page],
    queryFn: async () => {
      const params: Record<string, string> = { page: String(page) };
      if (statusFilter !== 'all') params.status = statusFilter;
      const res = await api.get('/v1/student/discussions/threads/', { params });
      return res.data;
    },
  });
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      return api.post('/v1/student/discussions/threads/create/', { title: newTitle, body: newBody });
    },
    onSuccess: () => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[22px] font-bold text-tp-text tracking-tight">Discussions</h1>
          <p className="mt-0.5 text-[13px] text-gray-400">
            {totalCount} {totalCount === 1 ? 'thread' : 'threads'} in your section
          </p>
        </div>
        <button
      ...
      {/* Filter */}
      <div className="flex items-center gap-0.5 rounded-xl bg-gray-50 border border-gray-100 p-0.5 w-fit">
        {(['all', 'open', 'closed'] as StatusFilter[]).map((s) => (
          <button
            ...
            className={cn(
              ...
              statusFilter === s ? 'bg-white text-tp-text shadow-sm' : 'text-gray-400 hover:text-gray-600',
            )}
          >
            {s}
          </button>
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1, 2, 3].map((i) => <div key={i} className="h-24 tp-skeleton rounded-2xl" />)}
        </div>
      ) : threads.length === 0 ? (
        <div className="text-center py-20">
          <div className="mx-auto text-gray-200 mb-4 flex justify-center"><EmptyIcon /></div>
          <h3 className="text-[15px] font-semibold text-tp-text mb-1">No discussions yet</h3>
          <p className="text-[13px] text-gray-400">Start a conversation by creating the first thread.</p>
        </div>
      ) : (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {(thread.course_title || thread.content_title) && (
                    <div className="flex flex-wrap items-center gap-1.5 mb-1.5">
                      {thread.course_title && (
                        <span className="px-2 py-[2px] rounded-md text-[10px] font-semibold bg-purple-50 text-purple-600 truncate max-w-[200px]">
                          {thread.course_title}
                        </span>
                      )}
                      {thread.content_title && (
                        <span className="px-2 py-[2px] rounded-md text-[10px] font-medium bg-gray-50 text-gray-500 truncate max-w-[200px]">
                          {thread.content_title}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    {thread.is_pinned && <span className="text-tp-accent flex-shrink-0" title="Pinned"><PinIcon /></span>}
                    <h3 className="text-[14px] font-semibold text-tp-text truncate group-hover:text-tp-accent transition-colors">
                      {thread.title}
                    </h3>
                  </div>

                  <p className="text-[12px] text-gray-400 leading-relaxed mb-2.5">{truncate(thread.body, 100)}</p>

                  <div className="flex flex-wrap items-center gap-3 text-[11px] text-gray-400">
                    <span className="font-medium text-gray-500">{thread.author.name}</span>
                    {thread.author.role && thread.author.role !== 'STUDENT' && (
                      <span className="px-1.5 py-[1px] rounded text-[9px] font-semibold uppercase bg-orange-50 text-orange-600">Teacher</span>
                    )}
                    <span>{relativeTime(thread.created_at)}</span>
                    <span className="inline-flex items-center gap-1"><ChatBubbleIcon /> {thread.reply_count}</span>
                    <span className="inline-flex items-center gap-1"><EyeIcon /> {thread.view_count}</span>
                  </div>
                </div>
        ...
        <div className="flex justify-center pt-2">
          <button
            ...
            className={cn(
              ...
              isFetching ? 'border-gray-100 text-gray-300 cursor-not-allowed' : 'border-gray-200 text-gray-600 hover:bg-gray-50',
            )}
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 p-6">
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-[17px] font-bold text-tp-text">New Discussion Thread</h2>
              <button onClick={() => setModalOpen(false)} className="text-gray-400 hover:text-gray-600 transition-colors"><XIcon /></button>
            </div>
            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                <label htmlFor="thread-title" className="block text-[12px] font-semibold text-gray-600 mb-1.5">Title</label>
                <input
                  id="thread-title"
                  type="text"
                  value={newTitle}
              ...
              <div>
                <label htmlFor="thread-body" className="block text-[12px] font-semibold text-gray-600 mb-1.5">Details</label>
                <textarea
                  id="thread-body"
                  value={newBody}
              ...
              <div className="flex justify-end gap-2 pt-1">
                <button type="button" onClick={() => setModalOpen(false)} className="px-4 py-2 rounded-xl text-[13px] font-medium text-gray-500 hover:text-gray-700 hover:bg-gray-50 transition-colors">
                  Cancel
                </button>
                <button
                  type="submit"
                  ...
                  className={cn(
                    ...
                    createMutation.isPending || !newTitle.trim() ? 'bg-gray-300 cursor-not-allowed' : 'bg-tp-accent hover:bg-tp-accent-dark',
                  )}
                >
